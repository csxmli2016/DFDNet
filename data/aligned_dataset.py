...
class AlignedDataset(BaseDataset):
    ...
    def WhichFilterGD(self,img):
        flag = random.random()
        if flag > 0.8:
            return self.UnMaskFilterGaussian(img)
        elif flag > 0.6:
    ...
    def UnMaskFilterGaussian(self,img):
        # if random.random() > 0.2: #一定概率不执行
        #     return img
        Rad = random.randint(3,11)# 3~15
        Perc = random.randint(60,170) #110
        # Thr = random.randint(0,5) #0~3
        Thr = 0
        dimg = img.filter(PIL.ImageFilter.UnsharpMask(radius=Rad, percent=Perc, threshold=Thr))
        return dimg    
    ...
    def GaussianBlur(self,img): # gaussian blur or motion blur
        img = np.array(img)
        blursize = random.randint(3,11) * 2 + 1 ##3,5,7,9,11,13,15
        blursigma = random.randint(1, 11)
        img = cv2.GaussianBlur(img, (blursize,blursize), blursigma/10)
        return Image.fromarray(img)
    ...
    def __getitem__(self, index): # 重构torch.util.data里的__getitem__函数
        ...
        # AB_path = './datasets/FFHQTrain/20963.png'
